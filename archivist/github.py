...
def backup_gh_repos(user, backupdir, token=None, backup_list=None, exclude_list=None):
    ...
    if token != None:
        ...
        response = requests.get(
            "https://api.github.com/user/repos?visibility=all", auth=(user, token)
        )
    ...
    for repo in to_backup:
        ...
        if localrepopath == None:
            ...
            pygit2.clone_repository(
                repo["clone_url"], str(dest), bare=True, callbacks=callbacks
            )
        else:
            log.info("Fetching updates...")
            localrepo = (
                pygit2.Repository(localrepopath)
                .remotes["origin"]
                .fetch(callbacks=callbacks)
            )
...
def backup_gh_gists(user, backupdir, token=None, backup_list=None, exclude_list=None):
    if token != None:
        response = requests.get(
            "https://api.github.com/users/" + user + "/repos", auth=(user, token)
        )
    else:
        response = requests.get("https://api.github.com/users/" + user + "/repos")
